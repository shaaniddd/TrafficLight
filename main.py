...
def detect(filepath,file):
    ...
    g_circles = cv2.HoughCircles(maskr, cv2.HOUGH_GRADIENT, 1, 80
                               param1=50, param2=10, minRadius=0, maxRadius=30)
    y_circles = cv2.HoughCircles(maskr, cv2.HOUGH_GRADIENT, 1, 80
                               param1=50, param2=10, minRadius=0, maxRadius=30)
    ...
    if r_circles is not None:
        ...
        for i in r_circles[0,:]:_
             
            if i[0] > size[1] or i[0] > size[0] or i[1] > size[0]*bound:
        ...
        for i in g_circles[0, :]:
            ...
            for m in range(-r, r):
                for n in range(-r, r):
                    ...
                    if (i[1]+m >= size[0] or (i[0]+n) >= szie[1]):
                        continue
                    h += maskg[i(1)+m, i[0]+n]
                    s += 1
            if h / s > 100:
    ...
    if y_circles is not None:
       y_circles = np.uint16(np.around(g_circles)) 
